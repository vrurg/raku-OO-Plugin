...
Most of the description for the functionality of this module can be found in L<C<OO::Plugin::Manual>|Manual.md>. Here we just cover
...
Read about L<sorting|Manual.md#sorting> in
L<C<OO::Plugin::Manual>|Manual.md>.
...
# Event dispatching attributes
has Async::Workers $!dispatch-mgr;
has Lock:D $!dmgr-lock .= new;
has Instant:D $!last-event-time = now;
has Promise $!timeout-monitor;
...
method event ( Str:D $name where { .chars > 0 }, |params ) {
    ...
    $!dmgr-lock.protect: {
        $!last-event-time = $ev-time if $!last-event-time < $ev-time;
        unless $!dispatch-mgr {
            $!dispatch-mgr = Async::Workers.new(:max($!event-workers));
            if $!ev-dispatcher-timeout > 0 {
                my sub set-timeout($timeout) {
                    ($!timeout-monitor = Promise.in($timeout)).then: {
                        my $now = now;
                        $!dmgr-lock.protect: {
                            my $delta = $!last-event-time + $!ev-dispatcher-timeout - $now;
                            if $delta > 0 {
                                set-timeout($delta);
                            }
                            else {
                                my $dmgr = $!dispatch-mgr;
                                $!dispatch-mgr = Nil;
                                $dmgr.shutdown;
                            }
                        }
                ...
                set-timeout($!ev-dispatcher-timeout);
            }
        }
        my @rc-promises;
        for self!plugins-cando('on-event', \($name, |params)) -> $pobj {
            @rc-promises.push: my $ppromise = Promise.new;
            $!dispatch-mgr.do-async({
                $ppromise.keep: [$pobj, $pobj.on-event($name, |params)];
                CATCH {
                    default {
                        $ppromise.break: X::OO::Plugin::Thrown.new(:plugin($pobj), :exception($_));
                    }
                }
            });
        }
        Promise.allof(@rc-promises).then: {
            $complete.keep: @rc-promises;
        }
    }
    $complete
}
...
in L<C<OO::Plugin::Manual>|Manual.md#basics>. This
...
method finish {
    ...
    $!dmgr-lock.protect: {
        unless $!finishing {
            $!finish-vow.keep: True;
            with $!dispatch-mgr {
                $mgr = $_;
            }
            else {
                # If there is no dispatch manager then we are done completely
                $!finished-vow.keep: True;
            }
        }
    }
    with $mgr {
...
method !topo-sort ( @mods ) {
    ...
    sub walk-elem ( $fqn, %SEEN is copy, :$depth = 0 ) {
        ...
        with %!dependencies{$fqn} -> %deps {
            ...
            msg "@@@ Checking deps of {$sn}: ", %deps.raku;
            for <demand after> -> $dt {
                msg ">> DEP TYPE $dt";
            ...
            DEPENDENCY:
                for %deps{$dt}.keys -> $p {
                    ...
                    given %SEEN{$p} {
                        ...
                        when Any:D {
                            die "Unexpected TPS seen mark value: " ~ $_.raku
                        }
                    }
        ...
        for @!order -> $plugin-fqn {
            ...
            with %methods{ $mname }{ $plugin-fqn } {
                for .keys -> $stage {
                    ...
                    my &routine = $_{ $stage };
                    self!dbg: "---- SIGNATURE: ", &routine.signature.raku;
...
L<C<OO::Plugin::Manual>|Manual.md>,
L<C<OO::Plugin>|../Plugin.md>,
L<C<OO::Plugin::Class>|Class.md>
L<C<OO::Plugin::Registry>|Registry.md>,
